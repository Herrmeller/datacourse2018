...
```{sql connection=con}
select name, elev from stations where elev > 400
```
...
```{sql connection=con}
select name from stations order by elev desc limit 5
```

```{sql connection=con}
select name from stations order by elev asc limit 5
```
...
```{sql connection=con}
select name, elev from stations where name like 'F%'
```

These are all stations starting with <span style="color: darkred">'F'</span>, while:

```{sql connection=con}
select name, elev from stations where name like '%burg%'
```
